...
- [ ] 5. iOS AcpClient parity (Swift; reuse existing openRawExec/SSHRawExecTransport + CodexLineTransport protocol; add minimal SharedKMPBridge surface for Acp* types/mapper/requests; thin client actor + focused smoke). **Next after item 10.** (iOS transport/docs already complete per inspection; main effort = bridge mirroring Codex pattern + client.)
- [x] 6. Focused smoke (AcpGrokClientSmokeTest with Canned + real mapper producing 5 CodexSessionItem kinds). (done — 1/1 green)
- [ ] 7. First minimal wiring: expose "connect as ACP/Grok" path (debug/new flow or toggle) in ViewModel(s) so real SSH + grok stdio can be driven. Feed mapped items to existing session state. **Codex connect/openAppServer paths byte-for-byte untouched.** (ServerRecord backendType is parked; keep minimal.)
