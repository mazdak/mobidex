...
class AcpProtocolCoreTest {
    ...
    @Test
    fun displayTitleDropsLocalCommandMarkup() {
        assertEquals("Fix the parser", acpDisplayTitle("Fix the parser"))
        assertEquals("a < b in the query planner", acpDisplayTitle("a < b in the query planner"))
        // The adapter truncates the raw first transcript entry, so a markup-only
        // title (Claude Code's local-command caveat) carries no usable text.
        assertNull(
            acpDisplayTitle(
                "<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond to the…"
            )
        )
        assertEquals("/review", acpDisplayTitle("<command-name>/review</command-name><command-args></command-args>"))
        assertEquals("/status", acpDisplayTitle("<local-command-caveat>Caveat…</local-command-caveat><command-name>/status"))
        assertNull(acpDisplayTitle("   "))
        assertNull(acpDisplayTitle(null))
    }

    @Test
    fun projectRootStripsClaudeWorktreeCheckouts() {
        assertEquals(
            "/Users/dev/Code/mobidex",
            acpProjectRoot("/Users/dev/Code/mobidex/.claude/worktrees/beautiful-bartik-3548a8"),
        )
        assertEquals(
            "/Users/dev/Code/mobidex",
            acpProjectRoot("/Users/dev/Code/mobidex/.claude/worktrees/x/nested/dir/"),
        )
        assertEquals("/Users/dev/Code/mobidex", acpProjectRoot("/Users/dev/Code/mobidex/"))
